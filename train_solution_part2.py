...
if __name__ == "__main__":
    ...
    lims = np.percentile(ttbar_true["Tau_pt"], [5, 95])
    bins = np.linspace(lims[0], lims[1], 30)
    plt.hist([ttbar_true["Tau_pt"],ttbar_fakes["Tau_pt"]], bins=bins, lw=3, histtype="step", label=["True taus","Jets"], normed=1)
    plt.xlabel("Tau_pt")
    plt.legend()
    plt.xlim((bins[0], bins[-1]))
    plt.tight_layout()
    plt.savefig("Tau_pt_ttbar.png")
